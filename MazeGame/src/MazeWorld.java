...
class Edge {
    ...
    WorldImage displayEdge(boolean visibleLine, boolean visiblePath) {
        ...
        if (visibleLine) {
            ...
            return new OverlayImages(this.from.displayCell(visiblePath), 
                    this.to.displayCell(visiblePath));
        }
    }
...
public class MazeWorld extends World {
    ...
    WorldImage lastImage() {
        return new OverlayImages(this.makeImage(), 
                new TextImage(new Posn(500, 300), "You win!", 180, 
                        new Color(255, 0, 0)));
    ...
    IList<Vertex> reconstruct(Vertex cur, 
            IList<Vertex> finalPath, HashMap<Vertex, Edge> cameFromEdge2) {
        if (cur.startVert) {
...
class ExamplesMaze {
    ...
    void testContains(Tester t) {
        IList<Integer> lII = new Cons<Integer>(10, new Cons<Integer>(3, new Cons<Integer>(5, 
                new Cons<Integer>(6, new Cons<Integer>(8, 
                        new Cons<Integer>(9, new Mt<Integer>()))))));
    ...
    void testDisplayEdge(Tester t) {
        ...
        t.checkExpect(eA.displayEdge(false, true), 
                new OverlayImages(vA.displayCell(true), vB.displayCell(true)));
        // vertically connected
        t.checkExpect(eB.displayEdge(true, true), new LineImage(new Posn(15, 5), 
                new Posn(15, 15), new Color(255, 0, 0)));
        t.checkExpect(eB.displayEdge(false, false), 
                new OverlayImages(vB.displayCell(false), vC.displayCell(false)));
    }
    ...
    void testDisplayWall(Tester t) {
        ...
        Edge eB = new Edge(vB, vC, 3);
        // horizontally connected
        t.checkExpect(eA.displayWall(), 
                new LineImage(new Posn(10, 0), new Posn(10, 10),
                        new Color(120, 0, 0)));
        // vertically connected
        t.checkExpect(eB.displayWall(),
                new LineImage(new Posn(10, 10), new Posn(20, 10),
                        new Color(120, 0, 0)));
    }  
    ...
    void testDisplayCell(Tester t) {
        ...
        Vertex vC = new Vertex(1, 1);
        vC.wasSearched = true;
        t.checkExpect(vA.displayCell(true), 
                new RectangleImage(new Posn(5, 5), 10, 10, new Color(205, 205, 205)));
        t.checkExpect(vB.displayCell(true), 
                new RectangleImage(new Posn(5, 15), 10, 10, new Color(65, 86, 197)));
        t.checkExpect(vC.displayCell(true), 
                new RectangleImage(new Posn(15, 15), 10, 10, new Color(56, 176, 222)));
    }
    ...
    void testFindNeighbors(Tester t) {
        this.initializeN();
        t.checkExpect(this.A.findNeighbors(), 
                new Cons<Vertex>(this.B, new Cons<Vertex>(this.E, new Mt<Vertex>())));
        t.checkExpect(this.B.findNeighbors(), 
                new Cons<Vertex>(this.A, new Cons<Vertex>(this.E, new Mt<Vertex>())));
        t.checkExpect(this.C.findNeighbors(), 
                new Cons<Vertex>(this.E, new Cons<Vertex>(this.D, new Mt<Vertex>())));
    }
